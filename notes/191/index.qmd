...
calculate_context_percentage() {
  ...
  ' 2>/dev/null) || usage_values=$'0\t0\t0'

  local input_tokens cache_read cache_creation
  IFS=$'\t' read -r input_tokens cache_read cache_creation <<< "$usage_values"

  local var_name
  for var_name in input_tokens cache_read cache_creation; do
...
Adjust the `repo_name` trimming logic if your repositories live outside `~/Repos/github.com/`.
